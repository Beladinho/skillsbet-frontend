...
function minimax(board, depth, alpha, beta, maximizing, winCount) {
  ...
  if (empty.length === 0) return { score: scoreBoard(board, winCount) };
  if (depth === 0) return { score: scoreBoard(board, winCount) };
  ...
  for (const [r, c] of empty) {
    ...
    newBoard[r][c] = player;
    if (checkWin(newBoard, r, c, player, winCount)) {
      return { score: maximizing ? 100000 + depth : -100000 - depth, row: r, col: c };
    }
    ...
    if (maximizing ? child.score > best.score : child.score < best.score) {
      best = { score: child.score, row: r, col: c };
    }
...
function getDepth(gridSize, difficulty) {
  if (gridSize === 3) return 9; // full tree for 3×3
  if (gridSize === 5) return difficulty === "expert" ? 6 : 4;
  return difficulty === "expert" ? 4 : 3; // 7×7
}
...
export default function XOBattleBotGame({ difficulty, playerId, onExit }) {
  ...
  useEffect(() => {
    if (currentPlayer === P2 && !winner && board && gridOption) {
      botTurn(board, gridOption);
    }
  ...
  return (
    <div style={{ textAlign: "center", marginTop: "20px", fontFamily: "'Rajdhani', 'Arial Narrow', sans-serif" }}>
      ...
      {!winner && (
        <p style={{
          color: thinking ? P2_COLOR : P1_COLOR,
          fontWeight: "bold",
          fontSize: "1.05rem",
          marginBottom: "14px",
          letterSpacing: "1px",
      ...
      {winner && (
        <div style={{ marginBottom: "16px" }}>
          <p style={{
            ...
            letterSpacing: "3px",
            textTransform: "uppercase",
            textShadow: winner !== "draw" ? `0 0 20px ${winner === P1 ? P1_COLOR : P2_COLOR}` : "none",
          }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: "12px", marginTop: "14px" }}>
            ...
            <button onClick={() => { setGridOption(null); setBoard(null); setWinner(null); }} className="btn-ghost" style={{ padding: "10px 28px", fontSize: "0.9rem" }}>Changer grille</button>
      ...
      <div style={{
        ...
        background: "#0a1628",
        border: `2px solid ${isHumanTurn ? P1_COLOR : thinking ? P2_COLOR : "#555"}`,
        borderRadius: "10px",
        padding: "12px",
        boxShadow: `0 0 24px rgba(${isHumanTurn ? "255,102,0" : thinking ? "0,180,216" : "0,0,0"},0.25), inset 0 0 40px rgba(0,0,0,0.4)`,
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: "36px", marginTop: "20px" }}>
        {[
          { color: P1_COLOR, symbol: "X", name: playerId, label: "Vous" },
          { color: P2_COLOR, symbol: "O", name: "BOT",     label: DIFFICULTY_LABELS[difficulty] },
        ].map(({ color, symbol, name, label }) => (
          <div key={name} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <span style={{ color, fontWeight: "bold", fontSize: "1.2rem", textShadow: `0 0 10px ${color}` }}>{symbol}</span>
            <span style={{ color: "#ccc", fontSize: "0.9rem" }}>
              {name} <span style={{ color: "#666", fontSize: "0.8rem" }}>({label})</span>
            </span>
          </div>
        ))}
      </div>
      ...
      {!winner && (
        <div style={{ display: "flex", justifyContent: "center", gap: "10px", marginTop: "18px" }}>
          <button
            onClick={() => { setGridOption(null); setBoard(null); setWinner(null); }}
            style={{ background: "transparent", border: "1px solid #333", color: "#666", borderRadius: "4px", padding: "7px 18px", cursor: "pointer", fontSize: "0.82rem", letterSpacing: "1px" }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = "#666"; e.currentTarget.style.color = "#aaa"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "#333"; e.currentTarget.style.color = "#666"; }}
          >
            Changer grille
          </button>
          <button onClick={onExit} className="btn-ghost" style={{ padding: "7px 18px", fontSize: "0.82rem" }}>
            ← Quitter
          </button>
        </div>
      )}
